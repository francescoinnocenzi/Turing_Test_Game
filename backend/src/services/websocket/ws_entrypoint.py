...
from services.game.session import get_data_session_ws
...
async def ws_entrypoint(websocket: WebSocket, room_name: str, client_id: str, cookie, backend: InMemoryBackend[UUID, SessionData], manager) -> None:
    ...
    if role not in {"PLAYER", "JUDGE"}:
        await websocket.close()
        raise ValueError(f"Ruolo non valido: {role}")


    print(f"Cookie disponibili: {websocket.cookies}")
    session_id, user_id = await get_data_session_ws(websocket, cookie, backend)
    ...
    if role == "PLAYER" and mode == "single":
        await auto_generate_next_question(room_name, session_id)

    try:
        while True:
            data = await websocket.receive_text() # contenuto del messaggio (stringa) ricevuto dal client 
            ws = manager.rooms[room_name][client_id]
            role = ws["role"] # ruolo del client che ha inviato il messaggio

            try:
                message = json.loads(data) # provo a convertirla in un dizionario 
                msg_type = message.get("type") # estrai dal dizionario il campo "type".
                print(f"Messaggio ricevuto: {message}")

                await handle_message(
                    msg_type, room_name, role, message,
                    mode, session_id, user_id, manager
                )

            except json.JSONDecodeError:
                raise HTTPException(status_code=400, detail="Messaggio non è un JSON valido")
