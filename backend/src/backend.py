...
def create_session(s: SessionRequest):
    conn = create_db_connection()
    cursor = conn.cursor()
    
    try:
        # Verifica se esiste già una sessione attiva per questa room
        cursor.execute("""
            SELECT id FROM sessions 
            WHERE room_name = ? AND status IN ('waiting', 'active')
        """, (s.room_name,))
        
        existing_session = cursor.fetchone()
        if existing_session:
            raise HTTPException(status_code=400, detail=f"Sessione già attiva per la room {s.room_name}")

        cursor.execute("""
            INSERT INTO sessions (room_name, judge_id, human_player_id, bot_player_id)
            VALUES (?, ?, ?, ?)
        """, (s.room_name, s.judge_id, s.human_player_id, s.bot_player_id))
        conn.commit()

        session_id = cursor.lastrowid

        cursor.execute("""
            SELECT id, room_name, judge_id, human_player_id, bot_player_id, status, created_at, completed_at
            FROM sessions
            WHERE id = ?
        """, (session_id,))
        row = cursor.fetchone()
        ...
        return SessionResponse(
            id=row[0], room_name=row[1], judge_id=row[2], human_player_id=row[3],
            bot_player_id=row[4], status=row[5], created_at=row[6], completed_at=row[7]
        )
...
@app.get("/sessions/{room_name}", response_model=SessionDetailsResponse)
def get_session_by_room(room_name: str):
    conn = create_db_connection()
    cursor = conn.cursor()
    
    try:
        cursor.execute("""
            SELECT session_id, room_name, judge_id, human_player_id, bot_player_id, 
                   status, session_created, completed_at, total_questions, total_answers
        ...
        row = cursor.fetchone()
        if not row:
            raise HTTPException(status_code=404, detail=f"Nessuna sessione attiva trovata per la room {room_name}")

        return SessionDetailsResponse(
            session_id=row[0], room_name=row[1], judge_id=row[2], human_player_id=row[3],
            bot_player_id=row[4], status=row[5], session_created=row[6], completed_at=row[7],
            total_questions=row[8], total_answers=row[9]
        )
...
@app.get("/sessions/", response_model=List[SessionDetailsResponse])
def get_all_sessions():
    ...
    try:
        ...
        for row in rows:
            sessions.append(SessionDetailsResponse(
                session_id=row[0], room_name=row[1], judge_id=row[2], human_player_id=row[3],
                bot_player_id=row[4], status=row[5], session_created=row[6], completed_at=row[7],
                total_questions=row[8], total_answers=row[9]
            ))
...
@app.get("/questions/session/{session_id}", response_model=List[QuestionResponse])
def get_questions_by_session(session_id: int):
    conn = create_db_connection()
    cursor = conn.cursor()
    
    try:
        cursor.execute("""
            SELECT id, text, session_id, author_id, room_name, created_at
            FROM questions
            WHERE session_id = ?
        ...
        for row in rows:
            questions.append(QuestionResponse(
                id=row[0], text=row[1], session_id=row[2],
                author_id=row[3], room_name=row[4], created_at=row[5]
            ))
    ...
    try:
        ...
        for row in rows:
            answers.append(AnswerResponse(
                id=row[0], question_id=row[1], session_id=row[2], text=row[3],
                author_id=row[4], author_type=row[5], room_name=row[6], created_at=row[7]
            ))
...
def complete_session(session_id: int):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            raise HTTPException(status_code=404, detail="Sessione non trovata o già completata")
        
        conn.commit()
        return {"message": f"Sessione {session_id} completata con successo"}
